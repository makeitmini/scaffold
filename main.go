...
func (t *testApp) updateFooter() {
	if t.showPosInfo {
		...
		t.layout.SetFooter("h: toggle help | p: show pos | q: quit | ↑/↓, j/k: line | u/d: half page | 1/2/3: align top/center/bottom")
	} else {
		// Default footer
		t.layout.SetFooter("Press 'h' for help | 'p' for position info | 'q' to quit | 1/2/3: alignment")
	}
}
...
func (t testApp) Update(msg tea.Msg) (tea.Model, tea.Cmd) {
	...
	case tea.KeyMsg:
		...
		case "1":
			// Set top alignment
			t.layout.SetVerticalAlignment(layout.AlignTop)
			return t, nil
		case "2":
			// Set center alignment
			t.layout.SetVerticalAlignment(layout.AlignCenter)
			return t, nil
		case "3":
			// Set bottom alignment
			t.layout.SetVerticalAlignment(layout.AlignBottom)
			return t, nil
		}
	}
...
func main() {
	...
	// Set vertical alignment - use one of AlignTop (default), AlignCenter, or AlignBottom
	// l.SetVerticalAlignment(layout.AlignCenter)  // Uncomment to test center alignment
	// l.SetVerticalAlignment(layout.AlignBottom)  // Uncomment to test bottom alignment
